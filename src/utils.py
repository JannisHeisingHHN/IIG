...
def get_gradient(model: Callable[[torch.Tensor], torch.Tensor], x: torch.Tensor) -> torch.Tensor:
    """
...
def randomize_model[T: nn.Module](model: T) -> T:
    ...
    for module in new_model.modules():
        if isinstance(module, nn.Conv2d):
            ...
            # In-projections
            if getattr(module, "_qkv_same_embed_dim", True):
                # Fused QKV
                ...
                # Optional biases may be stored as a single in_proj_bias in some builds, or
                # inside separate Linear wrappers in newer versions. Handle both:
                if hasattr(module, "in_proj_bias") and module.in_proj_bias is not None:
                    nn.init.constant_(module.in_proj_bias, 0.0)

            # Optional additive biases (rarely used)
            if hasattr(module, "bias_k") and module.bias_k is not None:
                nn.init.xavier_uniform_(module.bias_k)
            if hasattr(module, "bias_v") and module.bias_v is not None:
                nn.init.xavier_uniform_(module.bias_v)
